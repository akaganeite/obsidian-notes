...
- 消息类型
  - cantrip中消息类型固定，client只会发布消息
  - SUB类型消息：cantrip中完成特定任务的component是确定的。以完整性度量为例，该component与server通信的endpoint存在于双方cspace中，无需查找。那么就无需向server发送如SUB ima这样的消息，server收到mailbox发来的ima请求时直接通过camkes约定好的ep进行消息传递即可。
  - 同样，一个endpoint支持的RPCCall是单向的，不同endpoint上的通信可以区分消息类型，无需PUB/SUB这样的消息头来区分。
- 状态机
...
- interfacethread等待client事件，收到事件后解析并分发
  - 解析事件：在字典中查找
  - 分发事件：向对应的component endpoint调用rpc_basic_send，以预先约定的格式分发
