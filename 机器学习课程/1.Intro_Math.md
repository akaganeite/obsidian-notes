...
$$
其中第二个等号是由于分母和 $\theta$ 没有关系。求解这个 $\theta$ 值后计算$\frac{p(X|\theta)\cdot p(\theta)}{\int\limits _{\theta}p(X|\theta)\cdot p(\theta)d\theta}$ ，就得到了参数的后验概率。其中 $p(X|\theta)$ 叫似然，是
我们的模型分布。得到了参数的后验分布后，我们可以将这个分布用于预测贝叶斯预测。我们的目标是求出$p(x_{new}|X)$.以参数θ为桥梁来求解，具体过程可形象化的描述为X->θ->$x_{new}$.
$$
\begin{align}
p(x_{new}|X)&=\int\limits _{\theta}p(x_{new}|\theta,X)d\theta\ \ \ \ 这里通过求解边缘概率的积分引入\theta
\\&=\int\limits _{\theta}\bigg[\frac{p(x_{new},\theta,X)}{p(X)}d\theta=p(\theta|X)P(x_{new}|X,\theta)\bigg]d\theta\ \ \ X与X_{new}独立，分子根据chainrule变换
...
$$因为$(x-\mu)^{T}u_{i}$是一个标量，不妨令其为$y_i$.可以将$y_i$理解为$x-\mu$在特征向量$u_i$上的投影.上式子就是 $\Delta$ 取不同值时的同心椭圆,椭圆的长短半轴长由协方差矩阵特征值决定。
若协方差矩阵各向同性，则$\Delta$取不同值时对应的等高线为圆，若协方差矩阵为单位阵，则上述投影并无坐标变换。
