...
1.  硬分类，我们直接需要输出观测对应的分类。这类模型的代表为：
    1.  线性判别分析（Fisher 判别）(LDA,Linear Discirmination Analysis)
    2.  感知机
...
## 两分类-硬分类-感知机算法
感知机的核心思想为错误驱动，只可以解决线性可分的问题。
我们选取激活函数为：
$$
sign(a)=\left\{\begin{matrix}+1,a\ge0\\-1,a\lt0\end{matrix}\right.
$$
这样就可以将线性回归的结果映射到两分类的结果上了，具体式子如下：
$$f(x)=sign(w^Tx),x\in R^P$$$w^tx$大于0时属于类别1，y=1。
可以采用如下的方式定义是否分类正确，$w^Tx_i>0,y_i=1;w^Tx_i<0,y_i=-1$.综合上述二式得到当$y_iw^Tx_i>0$时分类正确。接下来，我们定义损失函数为错误分类的数目，比较直观的方式是使用指示函数，但是指示函数不可导，因此可以定义：
$$
...
## 两分类-硬分类-线性判别分析 LDA
LDA是一个经典分类算法模型，但有很大的局限性，现在已经很少实用该算法
在 LDA 中，我们的基本想法是选定一个方向，将试验样本顺着这个方向投影(投影至一维)，投影后的数据需要满足两个条件，从而可以更好地分类：
...
首先是投影，我们假定原来的数据是向量 $x$，那么顺着 $w$ 方向($||w||=1$)的投影就是标量：
$$
z=w^T\cdot x(=|w|\cdot|x|\cos\theta=|x|cos\theta=proj);\ \ \ \overline{z}=\frac{1}{n}\sum_{i=1}^Nw^Tx_i
$$
对第一点，相同类内部的样本更为接近，我们假设属于两类的试验样本数量分别是 $N_1$和 $N_2$，那么我们采用方差矩阵来表征每一个类内的总体分布，这里我们使用了协方差的定义，用 $S$ 表示原数据的协方差(样本协方差矩阵)：
$$
\begin{align}
C_1:Var_z[C_1](S_{c1})&=\frac{1}{N_1}\sum\limits_{i=1}^{N_1}(z_i-\overline{z_{c1}})(z_i-\overline{z_{c1}})^T\nonumber\\
...
&=w^TS_1w\\
C_2:Var_z[C_2](S_{c2})&=\frac{1}{N_2}\sum\limits_{i=1}^{N_2}(z_i-\overline{z_{c2}})(z_i-\overline{z_{c2}})^T\nonumber\\
&=w^TS_2w
\end{align}
$$
类内距离的方差用$S_w$表示，w即within class，需要被最小化：
$$
\begin{align}
Var_z[C_1]+Var_z[C_2]=w^T(S_1+S_2)w=w^TS_ww
\end{align}
$$
对于第二点，我们可以用两类的均值表示这个距离，需要被最大化：
$$
\begin{align}
...
&=w^T(\overline{x_{c1}}-\overline{x_{c2}})(\overline{x_{c1}}-\overline{x_{c2}})^Tw=w^TS_bw
\end{align}
$$
...
&=\mathop{argmax}\limits_w\frac{w^TS_bw}{w^TS_ww}=\mathop{argmax}\limits_w(w^TS_bw)(w^TS_ww)^{-1}
\end{align}
$$
...
&\Longrightarrow S_ww=\frac{w^TS_ww }{w^TS_bw}S_bw \ \ \ w^TS_bw,w^TS_ww是标量,令其为k\\
&\Longrightarrow w=kS_w^{-1}S_bw          \\
...
若类内协方差矩阵$S_w$为各向同性的对角矩阵，则$w\propto(\overline{x_{c1}}-\overline{x_{c2}})$.
...
生成模型不同于判别模型，它并不直接对$P(Y|X)$建模，而是借助贝叶斯定理$P(Y|X) \propto P(X|Y)P(Y)$.间接估计。换句话说，生成模型比较$P(y=0|x),P(y=1|x)$的大小，判别模型直接计算上述概率值
...
2. $P(x|y)=N(\mu_1,\Sigma)^y\cdot N(\mu_2,\Sigma)^{1-y}$
那么独立全同的数据集最大后验概率可以表示为：
$$
\begin{align}
L(\theta)=\mathop{argmax}_{\phi,\mu_0,\mu_1,\Sigma}\log p(X|Y)p(Y)=\mathop{argmax}_{\phi,\mu_0,\mu_1,\Sigma}\sum\limits_{i=1}^N (\log p(x_i|y_i)+\log p(y_i))\nonumber\\
...
*   然后求解 $\mu_1$：
    ...
    求微分左右同时乘以 $\Sigma$ 可以得到：
    $$
    \begin{align}\frac{L(\theta)}{\mu_1}=\sum\limits_{i=1}^N-2y_i\Sigma^{-1}x_i+2y_i\Sigma^{-1}\mu_1=0\nonumber\\\
...
*   最为困难的是求解 $\Sigma$，我们的模型假设对正反例采用相同的协方差矩阵，当然从上面的求解中我们可以看到，即使采用不同的矩阵也不会影响之前的三个参数。首先我们有：
    $$
    \begin{align}
    \sum\limits_{i=1}^N\big[\log\mathcal{N}(\mu,\Sigma)&=\sum\limits_{i=1}^N\log(\frac{1}{(2\pi)^{p/2}|\Sigma|^{1/2}})+(-\frac{1}{2}(x_i-\mu)^T\Sigma^{-1}(x_i-\mu))\nonumber\big]\\
    &=Const-\frac{1}{2}N\log|\Sigma|-\sum\limits_{i=1}^N\frac{1}{2}Trace((x_i-\mu)^T\Sigma^{-1}(x_i-\mu))\nonumber\\
    &=Const-\frac{1}{2}N\log|\Sigma|-\sum\limits_{i=1}^N\frac{1}{2}Trace((x_i-\mu)(x_i-\mu)^T\Sigma^{-1})\nonumber\\
