...
为了方便，我们首先将样本均值与样本协方差矩阵（数据集）写成中心化的形式：
$$
\begin{align}
\overline{X}&=\frac{1}{N}X^T\mathbb{I}_{N1}\\
...
&=\frac{1}{N}(X^T-\overline{X}\mathbb{I}_{1N}) (X^T-\overline{X}\mathbb{I}_{1N})^T   \\
...
\end{align}
$$
$\mathbb{I}_{N1}$是全部元素为1的列向量，$H_N=E_N-\frac{1}{N}\mathbb{I}_{N1}\mathbb{I}_{1N}$. 为centering matrix(中心矩阵)
中心矩阵拥有如下性质：1.$H^T=H$,2.$H^n=H$.上述推导的最后几步利用了中心矩阵$H$的性质。
...
1.  所有数据在子空间中更为分散 最大化投影方差
2.  损失的信息最小，即：在补空间的分量少 最小化重构距离
上述两点其实是从两个不同角度看到这个投影问题，最后会得到基本一致的解。最大化投影方差寻找到中心化后协方差矩阵最大的p个特征值，将其保留作为主成分。最小化重构距离将找出该协方差矩阵最小的q个特征值，并将对应维度丢弃。这里，p+q=n
...
上述过程从头推到如下：
1. 中心化 $X_i=X_i-\overline{X}$
2. 每个样本重构至新的空间，$X_i=\sum_{k=1}^p(x_i^T\cdot u_k)u_k$.
3. 选取q维，进行降维:$\hat{X_i}=\sum_{k=1}^q(x_i^T\cdot u_k)u_k$
4. 确定损失函数为最小化降维前后的距离:$J=\frac{1}{N}\sum_{i=1}^N||\hat{x_i}-x_i||^2$.
5. 其余步骤同上述过程一致，最后得到$J=\sum_{i=q+1}^p\lambda_i$.等价于挑选最小特征值
...
于是：
$$
S=\frac{1}{N}X^THX=\frac{1}{N}X^TH^THX=\frac{1}{N}V\Sigma^T\Sigma V^T=\frac{1}{N}V\Sigma^2V^T
$$
因此，我们直接对协方差矩阵进行特征值分解等价于对中心化后的数据直接SVD分解，都可以得到特征值和特征向量 $V$，在新坐标系中的坐标就是：
$$
HX\cdot V
$$
...
$$
\begin{align}
TU\Sigma&=U\Sigma(\Sigma^2)\\
HX\cdot V&=U\Sigma V^TV=U\Sigma
\end{align}
$$$U\Sigma$是T的特征向量矩阵，$\Sigma^2$是T的特征值矩阵，特征分解T可以直接得到坐标
