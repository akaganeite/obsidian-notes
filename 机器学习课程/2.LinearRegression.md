...
注意:本节讨论的均为机器学习算法，对优化问题无需迭代且可求出解析解。应用梯度下降算法的线性回归问题比本节所讨论的更为复杂，在本节中并不涉及。
本节仅以线性最小二乘为例，从多个角度看待该问题，阐述不同方法的区别与联系。
...
$$代表样本点到拟合曲线的误差总和，即我们希望拟合出的曲线尽可能的贴近每个样本点，展开得到：
$$
\begin{align}
...
\end{align}
$$这里，$w^TX^TY$是一个实数，可以直接和其转置合并为一项
最小化这个值的 $\hat{w}$ ：
$$
\begin{align}
...
$$
### 从另一个角度去看待最小二乘：
在几何上，最小二乘法相当于模型（这里就是直线）和试验值的距离的平方求和
假设我们的N个试验样本张成一个 $p$ 维空间（满秩的情况）：$X=Span(x_1,\cdots,x_N)$，用所有样本的线性组合来拟合y，那么模型可以写成 $f(w)=X\beta$，也就是 $x_1,\cdots,x_N$ 的某种组合，而最小二乘法就是说希望 $Y$ 和这个模型距离越小越好，于是它们的差应该与这个张成的空间垂直(几何意义是Y在样本点张成空间的投影，Y的维度通常高于p)：
$$
X^T\cdot(Y-X\beta)=0\longrightarrow\beta=(X^TX)^{-1}X^TY
$$和之前推导得到的结果是一样的。$\color{Orchid}{两种方法，第一种将误差分摊到每个样本点上，第二种将误差分摊在样本特征的每一维上}$.

接下来，我们将y看作一个随机变量，给定一个x，估计出y的分布。
## 噪声为高斯分布的 MLE

对于一维的情况，记 $y=w^Tx+\epsilon,\epsilon\sim\mathcal{N}(0,\sigma^2)$，那么 $y\sim\mathcal{N}(w^Tx,\sigma^2)$，也可以写作条件概率$p(y|x,w)\sim\mathcal{N}(w^Tx,\sigma^2)$,(将x,w看作常量，ε是随机变量，也即给定x,w的条件下，y是服从高斯分布的)代入极大似然估计中：
$$
\begin{align}
...
\end{align}
$$
这是从频率派角度看待该问题，采用最大似然求解，和最小二乘估计得到的结果一样

## 权重先验也为高斯分布的 MAP

现在从贝叶斯派的角度看待上述噪声最小二乘法，参数w是一个随机变量。取先验分布 $w\sim\mathcal{N}(0,\sigma_0^2)$。于是： 
$$
\begin{align}
\hat{w}=\mathop{argmax}\limits_wp(w|Y)=\mathop{argmax}\limits_w\frac{p(Y|w)\cdot p(w)}{p(Y)}&=\mathop{argmax}\limits_wp(Y|w)p(w)\nonumber\\
...
\end{align}
$$
这里省略了 $X$，$p(Y)$和 $w$ 没有关系，同时也利用了上面高斯分布的 MLE的结果。得到的结果相当于最小二乘法加上L2正则，$\frac{\sigma^2}{\sigma_0^2}$相当于是正则化中的λ
...
可以看到，这个正则化参数和前面的 MAP 结果不谋而合。利用2范数进行正则化不仅可以是模型选择 $w$ 较小的参数，同时也避免 $X^TX$不可逆的问题(需要求逆的矩阵现在变成了正定的)。
...
最小二乘=噪声为高斯的MLE
最小二乘+L2正则=权重，噪声均为高斯的MAP
= ：所求的参数w得到同样的结果，经过计算与变换后的优化目标一致
